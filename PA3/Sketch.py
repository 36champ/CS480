...
from ModelLinkage import Rabbit
from ModelLinkage import Tewi
from ModelLinkage import Mochi
...
class Sketch(CanvasBase):
    ...
    def InitGL(self):
        ...
        # Initialize
        # tewi = Tewi(self.vivarium.parent, Point((0, 0, 0)), self.vivarium.shaderProg)
        # tewi.initialize()
        # self.vivarium.addNewObjInTank(tewi)
        locations = [Point((1,0,0)), Point((0,1,0)), Point((0,0,1)), Point((1,1,0)), Point((1,0,1)), Point((0,1,1)),]
        for location in locations:
            rabbit = Rabbit(self.vivarium.parent, location, self.vivarium.shaderProg)
            rabbit.initialize()
            self.vivarium.addNewObjInTank(rabbit)
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        if chr(keycode) in "p":
            location = [random.random() * 3 - 1.5 for _ in  range(3)]
            rabbit = Rabbit(self.vivarium.parent, Point(location), self.vivarium.shaderProg)
            rabbit.initialize()
            self.vivarium.addNewObjInTank(rabbit)
            self.update()

        if chr(keycode) in "f":
            mochi = Mochi(self.vivarium.parent, Point((random.random() * 3 - 1.5, 1.5, random.random() * 3 - 1.5)), self.vivarium.shaderProg)
            mochi.initialize()
            self.vivarium.addNewObjInTank(mochi)
            self.update()
