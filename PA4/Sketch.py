...
class Sketch(CanvasBase):
    ...
    def __init__(self, parent):
        ...
        self.scenes = [SceneOne, SceneTwo, SceneFive, SceneThree, SceneFour]
        self.scene_index = 0
    ...
    def InitGL(self):
        ...
        # Light
        self.shaderProg.setBool("isAmbient", self.isAmbient)
        self.shaderProg.setBool("isDiffuse", self.isDiffuse)
        self.shaderProg.setBool("isSpecular", self.isSpecular)
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        if keycode in [wx.WXK_LEFT]:
            ...
            self.switchScene(self.scenes[self.scene_index](self.shaderProg))
            self.update()
        ...
        if chr(keycode) in "aA":
            self.isAmbient = not self.isAmbient
            self.shaderProg.setBool('isAmbient', self.isAmbient)
            self.update()
        if chr(keycode) in "dD":
            self.isDiffuse = not self.isDiffuse
            self.shaderProg.setBool('isDiffuse', self.isDiffuse)
            self.update()
        if chr(keycode) in "sS":
            self.isSpecular = not self.isSpecular
            self.shaderProg.setBool('isSpecular', self.isSpecular)
            self.update()

        # TODO 5.3 is at here
        if chr(keycode) in "<,":
            self.lightIndex = (self.lightIndex - 1) % len(self.lights)
        if chr(keycode) in ".>":
            self.lightIndex = (self.lightIndex + 1) % len(self.lights)
        if chr(keycode) in "lL":
            self.lights[self.lightIndex].lightOn = not self.lights[self.lightIndex].lightOn
            self.shaderProg.setBool(f"""{self.shaderProg.attribs["light"]}[{self.lightIndex}].lightOn""", self.lights[self.lightIndex].lightOn, False)
